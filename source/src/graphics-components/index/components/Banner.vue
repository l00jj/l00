...
class View {
  ...
  mainCamera = new THREE.PerspectiveCamera(75);
  ...
  constructor() {}
  async init(canvasEl: HTMLElement, containerEl: HTMLElement) {
    /**
     * 植入对象
     */
     ...
     * 创建渲染器
     */
    this.mainRenderer = new THREE.WebGLRenderer({ canvas: canvasEl, alpha: true });
     ...
     * 刷新视觉大小
     */
    this.resize(containerEl.offsetWidth, containerEl.offsetHeight);
    this.addTextToScene();
...
const onResize = () => {
  const containerEl: HTMLElement = container.value as HTMLElement;
  view.resize(containerEl.offsetWidth, containerEl.offsetHeight);
};

onMounted(() => {
  const canvasEl: HTMLElement = canvas.value as HTMLElement;
  const containerEl: HTMLElement = container.value as HTMLElement;
  view.init(canvasEl, containerEl);
  view.render();
...
<template>
  <div class="container" ref="container">
    ...
    <canvas class="" ref="canvas" v-show="isShowView"></canvas>
  </div>
</template>
