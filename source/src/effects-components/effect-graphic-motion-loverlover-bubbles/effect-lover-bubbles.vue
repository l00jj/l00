...
const bubbles = computed(() =>
  Array(count.value)
    ...
    .map(() => {
      ...
      return {
        size,
        style: `top:${y}px; left:${x}px; width:${size}px; height:${size}px; animation-duration:${
          1 * size
        }s; animation-delay:${size}s`,
      };
    })
);
</script>
  ...
  <section :class="[select]" :style="[color]">
    <div
      style="
        position: absolute;
        z-index: 999;
        background-image: url('./src/examples/css/effect-lover-bubbles/love.svg');
      "
    ></div>
    <div class="box" @click="change">
      <div class="circle">
        <h2 class="title"><span>Lover Bubbles For You</span><span>Heart Lover</span>77</h2>
      </div>
    </div>
    <div class="bubbles">
      <!--  -->
...
section .box .circle h2 span:nth-of-type(1) {
  font-size: 0.1em;
  line-height: 1;
  font-weight: 400;
}
section .box .circle h2 span:nth-of-type(2) {
  font-size: 0.2em;
  line-height: 1.2;
  font-weight: 400;
}
