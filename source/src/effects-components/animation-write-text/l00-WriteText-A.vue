...
const list = new Set<SVGPathElement>();

onMounted(() => {
  gsap.fromTo(
    [...list],
    {
      strokeDasharray: (index, target, targets) => target.getTotalLength() as number,
      strokeDashoffset: (index, target, targets) => target.getTotalLength() as number,
    },
    {
      strokeDashoffset: 0,
      repeat: -1,
...
<template>
  <section class="background">
    <h3>
      <svg class="subtitle" viewBox="0 0 229.2 69.6" style="enable-background: new 0 0 229.2 69.6">
        <defs>
        ...
        <path
          ...
          stroke="#FFFFFF"
          stroke-width="15"
          :ref="(el)=>list.add(el as SVGPathElement)"
        />
        <path
          class="path"
          clip-path="url(#websiteClip)"
